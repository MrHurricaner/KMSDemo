...
//        }
//
//        ECDSASignature sig = new NativeECDSASign().sign(messageHash);
...
//    }
//
public static Sign.SignatureData recoverFromSignature(ECDSASignature signature, byte[] messageHash, BigInteger publicKey) {

    int recId = -1;
    for (int i = 0; i < 4; i++) {
        BigInteger k = recoverFromSignature(i, signature, messageHash);
        if (k != null && k.equals(publicKey)) {
            recId = i;
            break;
        }
